...
export default function Home() {
  ...
  const { data: socialFollowers, loading } = useQuery(QUERY, {identity: address});
  ...
  useEffect(() => {
    ...
    const airstackFollowers = socialFollowers.SocialFollowers?.Follower?.map((item: any) => {
      return {
        ...
        name: item.followerAddress.domains.find((domain: any) => domain.isPrimary)?.name || item.followerAddress.identity,
    ...
    <main className="py-4 px-2">
      <Tabs defaultValue="recommended" className="space-y-4">
        ...
        <TabsContent value="recommended" className="space-y-4 pb-16">
          ...
          {followers.length > 0 && followers.map((item: any) =>
            <div key={`followers-${item.id}`}className="flex items-center justify-between w-full rounded-md p-2 transition-all hover:bg-accent hover:text-accent-foreground" onClick={() => router.push(`/${item.owner}`)}>
